...
public class RTrimCustomF extends BaseCustomFunction {
	...
	public Dataset<Row> execute(Dataset<Row> input) {
        ...
        input.sqlContext().udf().register("rtrim", udf, DataTypes.StringType);
        input = input.withColumn(super.newColName, callUDF("rtrim", col(super.colsToApply[0])));
